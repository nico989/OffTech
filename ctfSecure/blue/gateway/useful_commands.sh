...
# Run snort
sudo snort -c snort.conf -A console -Q -v
# community rules
tar -xvf /users/otech2ah/blue/gateway/snort/community.tar.gz -C ~/
...
# Read logs
snort -r /var/log/snort/snort.log.{id}
# Running snort in background
...
[Service]
Type=simple
ExecStart=/usr/local/bin/snort -q -u snort -g snort -c /etc/snort/snort.conf -i $ROUTER_ETH
...
# Rule against nmap
alert tcp any any -> $HOME_NET any (msg:"NMAP SCAN ON"; content:"nmap"; nocase; sid:10000001; rev:001;)

# sudo iptables -I FORWARD 1 -i eth1 -o eth4 -p tcp -d 10.1.5.2 -s 10.1.1.2 --dport 80 --tcp-flags ALL SYN -m conntrack --ctstate NEW  -j TCP-LIMIT
# sudo iptables -I FORWARD 8 -i eth1 -o eth4 -p tcp -d 10.1.5.2 -s 10.1.1.2 --dport 80 -m conntrack --ctstate ESTABLISHED -j ACCEPT
# sudo iptables -I FORWARD 12 -i eth4 -o eth1 -p tcp -s 10.1.5.2 -d 10.1.1.2 --sport 80 -m conntrack --ctstate ESTABLISHED -j ACCEPT
